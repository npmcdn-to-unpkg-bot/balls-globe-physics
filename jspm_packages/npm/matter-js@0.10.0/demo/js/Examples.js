...
var _isBrowser = typeof window !== 'undefined' && window.location,
    Matter = _isBrowser ? window.Matter : require('../../build/matter-dev');
var Example = {};
...
(function() {
  ...
  Example.airFriction = function(demo) {
    var engine = demo.engine,
        world = engine.world;
    World.add(world, [Bodies.rectangle(200, 100, 60, 60, {frictionAir: 0.001}), Bodies.rectangle(400, 100, 60, 60, {frictionAir: 0.05}), Bodies.rectangle(600, 100, 60, 60, {frictionAir: 0.1})]);
...
(function() {
  ...
  Example.ballPool = function(demo) {
    ...
    World.add(world, [stack, Bodies.polygon(200, 560, 3, 60), Bodies.polygon(400, 560, 5, 60), Bodies.rectangle(600, 560, 80, 80)]);
  };
})();
(function() {
  var World = Matter.World,
...
(function() {
  ...
  Example.bridge = function(demo) {
    ...
    World.add(world, [bridge, Bodies.rectangle(80, 440, 120, 280, {isStatic: true}), Bodies.rectangle(720, 440, 120, 280, {isStatic: true}), Constraint.create({
      pointA: {
        x: 140,
        y: 300
      },
...
(function() {
  ...
  Example.car = function(demo) {
    ...
    World.add(world, [Bodies.rectangle(200, 150, 650, 20, {
      isStatic: true,
      angle: Math.PI * 0.06
...
(function() {
  ...
  Example.catapult = function(demo) {
    ...
    World.add(world, [stack, catapult, Bodies.rectangle(250, 555, 20, 50, {isStatic: true}), Bodies.circle(560, 100, 50, {density: 0.005}), Constraint.create({
      bodyA: catapult,
      pointB: {
        x: 390,
        y: 580
...
(function() {
  ...
  Example.cloth = function(demo) {
    ...
    World.add(world, [cloth, Bodies.circle(300, 500, 80, {isStatic: true}), Bodies.rectangle(500, 480, 80, 80, {isStatic: true})]);
  };
})();
(function() {
  var World = Matter.World,
  ...
  Example.collisionFiltering = function(demo) {
    ...
    var redColor = '#C44D58',
        blueColor = '#4ECDC4',
        greenColor = '#C7F464';
    World.add(world, Composites.stack(275, 150, 5, 10, 10, 10, function(x, y, column, row) {
...
(function() {
  ...
  Example.compositeManipulation = function(demo) {
    ...
    World.add(world, stack);
    world.gravity.y = 0;
    sceneEvents.push(Events.on(engine, 'afterUpdate', function(event) {
...
(function() {
  ...
  Example.compound = function(demo) {
    ...
    var compoundBodyA = Body.create({parts: [partA, partB]});
    size = 150;
    x = 400;
    y = 300;
    ...
    var compoundBodyB = Body.create({parts: [partC, partD, partE, partF]});
  ...
  Example.compoundStack = function(demo) {
    ...
    var stack = Composites.stack(100, 280, 12, 6, 0, 0, function(x, y) {
      var partA = Bodies.rectangle(x, y, size, size / 5),
          partB = Bodies.rectangle(x, y, size / 5, size, {render: partA.render});
      return Body.create({parts: [partA, partB]});
    });
    World.add(world, stack);
  };
})();
(function() {
  ...
  Example.concave = function(demo) {
    ...
    var stack = Composites.stack(50, 50, 6, 4, 10, 10, function(x, y) {
      ...
      return Bodies.fromVertices(x, y, Common.choose([arrow, chevron, star, horseShoe]), {render: {
          fillStyle: color,
          strokeStyle: color
        }}, true);
    });
...
(function() {
  ...
  Example.events = function(demo) {
    var engine = demo.engine,
        ...
        mouseConstraint = demo.mouseConstraint,
        sceneEvents = demo.sceneEvents;
    sceneEvents.push(Events.on(world, 'afterAdd', function(event) {
      console.log('added to world:', event.object);
    }));
    sceneEvents.push(Events.on(engine, 'beforeUpdate', function(event) {
    ...
    sceneEvents.push(Events.on(engine, 'collisionActive', function(event) {
      var pairs = event.pairs;
      for (var i = 0; i < pairs.length; i++) {
        var pair = pairs[i];
    ...
    sceneEvents.push(Events.on(engine, 'collisionEnd', function(event) {
      var pairs = event.pairs;
      for (var i = 0; i < pairs.length; i++) {
        var pair = pairs[i];
    ...
    sceneEvents.push(Events.on(mouseConstraint, 'mousedown', function(event) {
      var mousePosition = event.mouse.position;
      console.log('mousedown at ' + mousePosition.x + ' ' + mousePosition.y);
      demo.render.options.background = 'cornsilk';
      shakeScene(engine);
    }));
    sceneEvents.push(Events.on(mouseConstraint, 'mouseup', function(event) {
      var mousePosition = event.mouse.position;
      demo.render.options.background = "white";
      console.log('mouseup at ' + mousePosition.x + ' ' + mousePosition.y);
    }));
    sceneEvents.push(Events.on(mouseConstraint, 'startdrag', function(event) {
      console.log('startdrag', event);
    }));
    sceneEvents.push(Events.on(mouseConstraint, 'enddrag', function(event) {
      console.log('enddrag', event);
    }));
    var stack = Composites.stack(50, 100, 8, 4, 50, 50, function(x, y) {
      return Bodies.circle(x, y, 15, {
        restitution: 1,
        render: {strokeStyle: '#777'}
      });
    });
    World.add(world, stack);
    var renderOptions = demo.render.options;
...
(function() {
  ...
  Example.raycasting = function(demo) {
    ...
    sceneEvents.push(Events.on(demo.render, 'afterRender', function() {
...
(function() {
  ...
  Example.restitution = function(demo) {
    ...
    var rest = 0.9,
        space = 600 / 5;
    World.add(world, [Bodies.rectangle(100 + space * 0, 150, 50, 50, {restitution: rest}), Bodies.rectangle(100 + space * 1, 150, 50, 50, {
      restitution: rest,
      angle: -Math.PI * 0.15
    }), Bodies.rectangle(100 + space * 2, 150, 50, 50, {
      restitution: rest,
      angle: -Math.PI * 0.25
    }), Bodies.circle(100 + space * 3, 150, 25, {restitution: rest}), Bodies.rectangle(100 + space * 5, 150, 180, 20, {
      restitution: rest,
      angle: -Math.PI * 0.5
    })]);
    var renderOptions = demo.render.options;
...
(function() {
  ...
  Example.rounded = function(demo) {
    var engine = demo.engine,
        world = engine.world;
    World.add(world, [Bodies.rectangle(200, 200, 100, 100, {chamfer: {radius: 20}}), Bodies.rectangle(300, 200, 100, 100, {chamfer: {radius: [90, 0, 0, 0]}}), Bodies.rectangle(400, 200, 200, 200, {chamfer: {radius: [150, 20, 40, 20]}}), Bodies.rectangle(200, 200, 200, 200, {chamfer: {radius: [150, 20, 150, 20]}}), Bodies.rectangle(300, 200, 200, 50, {chamfer: {radius: [25, 25, 0, 0]}}), Bodies.polygon(200, 100, 8, 80, {chamfer: {radius: 30}}), Bodies.polygon(300, 100, 5, 80, {chamfer: {radius: [10, 40, 20, 40, 10]}}), Bodies.polygon(400, 200, 3, 50, {chamfer: {radius: [20, 0, 20]}})]);
    var renderOptions = demo.render.options;
    renderOptions.showAxes = true;
    renderOptions.showCollisions = true;
  };
...
(function() {
  ...
  Example.sensors = function(demo) {
    ...
    World.add(world, Bodies.circle(400, 40, 30, {render: {
...
(function() {
  ...
  Example.softBody = function(demo) {
    ...
    World.add(world, [Composites.softBody(250, 100, 5, 5, 0, 0, true, 18, particleOptions), Composites.softBody(400, 300, 8, 3, 0, 0, true, 15, particleOptions), Composites.softBody(250, 400, 4, 4, 0, 0, true, 15, particleOptions)]);
    var renderOptions = demo.render.options;
    renderOptions.showAngleIndicator = false;
  };
})();
  ...
  Example.sprites = function(demo) {
    ...
    var stack = Composites.stack(20, 20, 10, 4, 0, 0, function(x, y) {
      if (Common.random() > 0.35) {
        return Bodies.rectangle(x, y, 64, 64, {render: {
            strokeStyle: '#ffffff',
            sprite: {texture: './img/box.png'}
          }});
      } else {
        return Bodies.circle(x, y, 46, {
          ...
          restitution: 0.3,
          friction: 0.01,
          render: {sprite: {texture: './img/ball.png'}}
        });
      }
    });
    World.add(world, stack);
...
(function() {
  ...
  Example.svg = function(demo) {
    ...
    for (var i = 0; i < svgs.length; i += 1) {
      (function(i) {
        $.get('./svg/' + svgs[i] + '.svg').done(function(data) {
          ...
          World.add(world, Bodies.fromVertices(100 + i * 150, 200 + i * 50, vertexSets, {render: {
              fillStyle: color,
              strokeStyle: color
            }}, true));
        });
    ...
    $.get('./svg/svg.svg').done(function(data) {
      ...
      World.add(world, Bodies.fromVertices(400, 80, vertexSets, {render: {
          fillStyle: color,
          strokeStyle: color
        }}, true));
    });
...
(function() {
  ...
  Example.views = function(demo) {
    ...
    sceneEvents.push(Events.on(engine, 'beforeTick', function() {
